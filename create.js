...
{
  let [createpost,useractivity]=await Promise.all([createpost({ title: 'post 3', body: 'yeah this is post 3' }),updateLastUserActivityTime()]);

  console.log(`before creating ${posts[posts.length-1].title}`,'user activity time'+ usertime[usertime.length-2]);
  getpost();
  console.log('last activity time of user'+`${usertime[usertime.length-1]}`)
  let letsdeletepost=await deletepost();
  console.log(posts)
